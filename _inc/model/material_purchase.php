...
| -----------------------------------------------------
*/
class ModelMaterial_Purchase extends Model 
{
	public function getInvoices($type, $store_id = null, $limit = 100000) 
	{
		$store_id = $store_id ? $store_id : store_id();
		$statement = $this->db->prepare("SELECT `material_purchase_info`.*, `material_purchase_price`.*, `suppliers`.`sup_id`, `suppliers`.`sup_name`, `suppliers`.`sup_mobile`, `suppliers`.`sup_email` FROM `material_purchase_info` 
			...
			WHERE `material_purchase_info`.`store_id` = ? AND `material_purchase_info`.`inv_type` = ? ORDER BY `material_purchase_info`.`created_at` DESC LIMIT $limit");
	...
	{
		$store_id = $store_id ? $store_id : store_id();
		$statement = $this->db->prepare("SELECT * FROM `material_purchase_item` WHERE `store_id` = ? AND `invoice_id` = ?");
	...
	{
		$store_id = $store_id ? $store_id : store_id();
		$statement = $this->db->prepare("SELECT * FROM `material_purchase_item` WHERE `store_id` = ? AND `invoice_id` = ? AND `item_id` = ?");
	...
	{
		$statement = $this->db->prepare("SELECT SUM(`item_quantity`) as qty, SUM(`tax`) as tax, SUM(`item_total`) as total, item_tax, `taxrates`.`taxrate_name`, `taxrates`.`code_name` FROM `material_purchase_item` LEFT JOIN `taxrates` ON (`material_purchase_item`.`taxrate_id` = `taxrates`.`taxrate_id`) WHERE invoice_id = ? GROUP BY `material_purchase_item`.`taxrate_id`");
	...
	{
		$store_id = $store_id ? $store_id : store_id();
		$statement = $this->db->prepare("SELECT * FROM `material_purchase_info` WHERE `store_id` = ? AND invoice_id = ?");
		$statement->execute(array($store_id, $invoice_id));
		return $statement->fetch(PDO::FETCH_ASSOC);
	}
	...
	{
		$store_id = $store_id ? $store_id : store_id();
		$statement = $this->db->prepare("SELECT * FROM `material_purchase_info` WHERE `material_purchase_info`.`store_id` = ? AND `material_purchase_info`.`invoice_id` = ?");
		$statement->execute(array($store_id, $invoice_id));
	...
	{
		$store_id = $store_id ? $store_id : store_id();
		$statement = $this->db->prepare("DELETE FROM `material_purchase_item` WHERE `store_id` = ? AND `invoice_id` = ? AND `item_id` = ?");
		$statement->execute(array($store_id, $invoice_id, $item_id));
		return true;
	}
	...
	{
		$store_id = $store_id ? $store_id : store_id();
		$statemtnt = $this->db->prepare("SELECT * FROM `material_purchase_info` WHERE `store_id` = ? AND `sup_id` = ? AND `inv_type` IN ('purchase','transfer') ORDER BY `info_id` DESC LIMIT 1");
	...
	{
		$store_id = $store_id ? $store_id : store_id();
		$statemtnt = $this->db->prepare("SELECT * FROM `material_purchase_info` WHERE `store_id` = ? AND `inv_type` = ? ORDER BY `info_id` DESC");
	...
	{
		$store_id = $store_id ? $store_id : store_id();
		$statemtnt = $this->db->prepare("SELECT * FROM `material_purchase_info` WHERE `store_id` = ? AND `sup_id` = ? AND `inv_type` = ? ORDER BY `info_id` DESC");
	...
	public function totalToday($store_id = null)
	{
		...
		$statement = $this->db->prepare("SELECT * FROM `material_purchase_info` WHERE $where_query");
